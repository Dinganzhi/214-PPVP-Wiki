...
export default defineConfig({
            ...
            items: [
              ...
              },
              {
                ...
                items: [
                  ...
                  { text: "特殊区域-scp67区域", link: "/ppvp/maps/second_map/scp67" },
  ...
  // ========== Vite 配置：强制替换桌面端 "三个点" 菜单组件 ==========
  vite: {
    plugins: [
      {
        name: 'force-replace-navbar-extra',
        enforce: 'pre',
        transform(code, id) {
          if (id.endsWith('VPNavBar.vue')) {
            const customPath = path.resolve(__dirname, './theme/components/VPNavBarExtra.vue')
            return code.replace(
              /from\s+['"]\.\/VPNavBarExtra(\.vue)?['"]/,
              `from "${customPath}"`
            )
          }
          return null
